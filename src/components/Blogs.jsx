...
const Blogs = () => {
  ...
  const fetchRequestData = useSelector(
    (state) => state?.counter?.listPost?.result
  );
  const fetchRequestPending = useSelector(
    (state) => state?.counter?.listPost?.fetching
  );
  const fetchRequestError = useSelector(
    (state) => state?.counter?.listPost?.error
  );
  ...
  return (
    <>
      ...
      <div className="blog-container">
        ...
        {fetchRequestData < 1 && !fetchRequestPending ? (
          <p>{fetchRequestError}</p>
        ) : null}

        {fetchRequestPending ? (
          <div className="spinner ml-96  mt-44 mb-44 m-auto">
            <button
              disabled
              type="button"
